var bootText = `[  OK  ] Started Show Plymouth Boot Screen.
...
function displayNextLine() {
    if (lineIndex < bootLines.length) {
        ...
        okSpan.className = 'ok';
        if (line.includes('OK')) {
            var parts = line.split('OK');
            normalSpan.textContent = parts[0];
            okSpan.textContent = 'OK';
            document.getElementById('boot-sequence').appendChild(normalSpan);
            document.getElementById('boot-sequence').appendChild(okSpan);
            if (parts[1]) {
                normalSpan = document.createElement('span');
                normalSpan.className = 'normal';
                normalSpan.textContent = parts[1];
                document.getElementById('boot-sequence').appendChild(normalSpan);
            }
        } else if (line.includes('Arch Linux')) {
            var parts = line.split('Arch Linux');
            normalSpan.textContent = parts[0];
            archSpan = document.createElement('span');
            archSpan.className = 'arch';
            archSpan.textContent = 'Arch Linux';
            document.getElementById('boot-sequence').appendChild(normalSpan);
            document.getElementById('boot-sequence').appendChild(archSpan);
            if (parts[1]) {
                normalSpan = document.createElement('span');
                normalSpan.className = 'normal';
                normalSpan.textContent = parts[1];
            ...
            normalSpan.textContent = line;
            document.getElementById('boot-sequence').appendChild(normalSpan);
        }
        document.getElementById('boot-sequence').appendChild(document.createTextNode('\n'));

        console.log(lineIndex % 17);

        if (lineIndex % 20 == 0 && firstTime == false) {
            firstTime = true;
            setTimeout(displayNextLine, 1000);
        } else if (lineIndex % 17 == 0) {
            setTimeout(displayNextLine, 125);
        } else {
